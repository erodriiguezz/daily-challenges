...
function getMovieNightCost(day, showtime, tickets) {
  ...
  if (normalizedDay === "tuesday") {
    var totalTuesday = tickets * 5;
    return "$" + totalTuesday.toFixed(2);
  }

  // base price
  const isWeekend = ["friday", "saturday", "sunday"].includes(normalizedDay);
  let pricePerTicket = isWeekend ? 12 : 10;

  // check time and convert to 24 hours
  let hour = parseInt(showtime.split(":")[0]);

  if (showtime.includes("pm")) hour += 12;
  if (showtime.includes("am")) hour += 0;
