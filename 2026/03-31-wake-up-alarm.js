...
function alarmCheck(alarmTime, wakeTime) {
  ...
  let alarmMinutes = parseInt(alarmTime.split(":")[1]);

  let wakeHour = parseInt(wakeTime.split(":")[0]);
  let wakeMinutes = parseInt(wakeTime.split(":")[1]);

  const alarmInMinutes = alarmHour * 60 + alarmMinutes;
  const wakeInMinutes = wakeHour * 60 + wakeMinutes;

  if (wakeInMinutes < alarmInMinutes) return "early";
  if (wakeInMinutes === alarmInMinutes || wakeInMinutes <= alarmInMinutes + 10)
    return "on time";
  if (wakeInMinutes > alarmInMinutes + 10) return "late";
}
